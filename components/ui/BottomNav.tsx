...
export default function BottomNav({ active, onChange }: BottomNavProps) {
  return (
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] border-t border-v-border flex shadow-[0_-2px_16px_rgba(0,0,0,0.04)] z-[100]"
      style={{ background: "linear-gradient(to right, #4A4630, #332F1F)", paddingBottom: "max(env(safe-area-inset-bottom), 6px)" }}>
      {tabs.map((t) => {
        const isActive = active === t.id;
        return (
          <button key={t.id} onClick={() => onChange(t.id)}
            className={`flex-1 flex flex-col items-center gap-0.5 py-1.5 border-none bg-none transition-colors ${isActive ? "text-white" : "text-white/60"}`}>
            {t.icon("currentColor")}
            <span className={`text-[9px] ${isActive ? "font-bold" : "font-medium"}`}>{t.label}</span>
            <span className={`w-1 h-1 rounded-full mt-0.5 ${isActive ? "bg-[#E8792B]" : "bg-transparent"}`} />
          </button>
        );
      })}
    </div>
